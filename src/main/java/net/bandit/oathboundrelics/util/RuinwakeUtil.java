...
public final class RuinwakeUtil {
    ...
    public static int getGrudge(Player player) {
        return Math.clamp(
                player.getPersistentData().getInt(GRUDGE_KEY),
                0,
    ...
    public static void addGrudge(Player player, int amount) {
        ...
        player.getPersistentData().putInt(GRUDGE_KEY, next);
        markCombat(player);
    }

    public static int consumeGrudge(Player player) {
        int current = getGrudge(player);
        player.getPersistentData().putInt(GRUDGE_KEY, 0);
        return current;
    }
    ...
    public static void markCombat(Player player) {
        ...
        player.getPersistentData().putLong(LAST_COMBAT_KEY, now);
        player.getPersistentData().putLong(LAST_DECAY_KEY, now);
    }
    ...
    public static void decayGrudge(Player player) {
        ...
        long now = player.level().getGameTime();
        long lastCombat = player.getPersistentData().getLong(LAST_COMBAT_KEY);
        long lastDecay = player.getPersistentData().getLong(LAST_DECAY_KEY);
        ...
        player.getPersistentData().putInt(GRUDGE_KEY, stacks - 1);
        player.getPersistentData().putLong(LAST_DECAY_KEY, now);
    }

    public static boolean isReleasing(Player player) {
        return player.getPersistentData().getBoolean(RELEASING_KEY);
    }

    public static void setReleasing(Player player, boolean releasing) {
        player.getPersistentData().putBoolean(RELEASING_KEY, releasing);
    }
